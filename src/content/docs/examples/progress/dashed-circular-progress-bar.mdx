...
description: A segmented dashed circular progress bar with Orbit and CSS. Ten color-fading dashes around a center value. Copy the markup and theme the segments.
---
...
- **No application JavaScript** is needed for a static state beyond loading the Orbit runtime. To make the bar respond to a value, swap arc classes via JS: each arc represents 10%, so `value=70` means the first 7 arcs are `--o-blue` and the rest are `--o-blue-lighter`.
