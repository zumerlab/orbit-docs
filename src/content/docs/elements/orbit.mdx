...
The `.orbit` CSS class, along with explicit integer classes such as `.orbit-0`, `.orbit-6` and `.orbit-24`, defines a circular path around a `.gravity-spot`. Within  `.orbit` element, various **Orbit** elements such as `satellites`, `o-arcs`, `o-progress`, `vectors` and `sides` can be positioned.
...
**Orbit** supports automatic `.orbit` numbering and explicit nonnegative integer ring levels. Levels **0** to **12** lie within the base `--o-force` diameter; higher levels extend beyond it. The JavaScript runtime supports more than 24 levels and more than 60 children per type. Those limits apply only to the static CSS fallback.
...
- Orbits from **13** onward are situated beyond `.gravity-spot` force.
