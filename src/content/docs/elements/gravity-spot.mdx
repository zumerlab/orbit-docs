...
:::note
For responsive designs, consider using the optional `Orbit.resize()` JavaScript function, which adjusts `--o-force-ratio` dynamically when the parent container is resized. It returns a cleanup function to call when that container unmounts.
:::
